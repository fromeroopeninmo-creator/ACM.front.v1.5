...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!allowed) return jsonError("No tenés permisos para eliminar este evento.", 403);

    if (!activity.google_calendar_event_id) {
      return NextResponse.json({ ok: true, deleted: false, reason: "La actividad no tenía evento de Google vinculado." });
    }
    ...
    if (!res.ok && res.status !== 404 && res.status !== 410) {
      ...
      const msg = googleJson?.error?.message || "No se pudo eliminar el evento en Google Calendar.";
    ...
    return NextResponse.json({ ok: true, deleted: true, activity: updated });
  } catch (err: any) {
    console.error("Google delete event error:", err);
    return jsonError(err?.message || "No se pudo eliminar el evento de Google Calendar.", 500);
  }
}
