...
const TEMPLATE_OPTIONS: Array<{ id: PlateTemplateId; name: string; detail: string }> = [
  { id: "hero", name: "Foto protagonista", detail: "Imagen completa con degradado" },
  { id: "bottom", name: "Ficha inferior", detail: "Foto amplia y datos al pie" },
  { id: "side", name: "Lateral corporativa", detail: "Foto protagonista con panel lateral" },
  { id: "mosaic2", name: "Mosaico 2 fotos", detail: "Una principal y una secundaria" },
  { id: "mosaic3", name: "Mosaico 3 fotos", detail: "Una principal y dos detalles" },
  { id: "minimal", name: "Minimal", detail: "Mucho aire y datos esenciales" },
];
...
export default function PlateEditor({ mode }: { mode: "empresa" | "asesor" }) {
      ...
      try {
        ...
        const professionalName =
          mode === "asesor"
            ...
            : String(company?.matriculado || profile.matriculado_nombre || company?.nombre_comercial || "Profesional inmobiliario");
        ...
        setMessage("No se pudo cargar toda la identidad corporativa. Podés continuar editando manualmente.");
      } finally {
  ...
  useEffect(() => {
    return () => {
      ...
      if (state.importedPlate) URL.revokeObjectURL(state.importedPlate.url);
    };
  }, [state.images, state.importedPlate]);
  ...
  const accentText = contrastText(state.primaryColor);

  const update = <K extends keyof PlateState>(key: K, value: PlateState[K]) => {
    setState((prev) => ({ ...prev, [key]: value }));
  };

  const handleImages = (files: FileList | null, imported = false) => {
    ...
    if (imported) {
      ...
      setState((prev) => {
        if (prev.importedPlate) URL.revokeObjectURL(prev.importedPlate.url);
        return {
          ...prev,
          importedPlate: { id: crypto.randomUUID(), name: file.name, url: URL.createObjectURL(file) },
          useImportedPlateMode: true,
        };
      });
      return;
    }
    const next = accepted.slice(0, MAX_IMAGES).map((file) => ({
      id: crypto.randomUUID(),
      name: file.name,
      url: URL.createObjectURL(file),
    }));
    setState((prev) => {
      prev.images.forEach((image) => URL.revokeObjectURL(image.url));
      return { ...prev, images: next, useImportedPlateMode: false };
    });
  };
  ...
  const download = async (kind: "png" | "jpg") => {
    ...
    try {
      ...
      const dataUrl = kind === "png"
        ...
        : await toJpeg(canvasRef.current, { ...common, quality: 0.94, backgroundColor: "#ffffff" });
      ...
      setMessage("No se pudo generar la imagen. Probá con fotos más livianas o en formato JPG/PNG.");
    } finally {
      setExporting(false);
    ...
    <div className={`relative overflow-hidden bg-slate-200 ${className}`}>
      {image ? (
        <img
          ...
          style={{ objectPosition: objectPosition(state.imagePosition), transform: `scale(${state.imageZoom})` }}
        />
      ) : (
  ...
  const StatusStamp = () => state.showStatus && state.status ? (
    <div
      className="absolute right-[4%] top-[5%] z-30 rotate-[-8deg] border-[6px] px-6 py-3 text-center font-black uppercase tracking-[0.18em] shadow-lg"
      style={{ borderColor: state.primaryColor, color: state.primaryColor, backgroundColor: "rgba(255,255,255,.9)" }}
    >
      {state.status}
    </div>
  ) : null;

  const PriceBlock = ({ dark = false }: { dark?: boolean }) => state.showPrice ? (
    <div className="space-y-1">
      {state.showOldPrice && state.oldPrice ? (
        <div className={`text-lg font-bold line-through opacity-70 ${dark ? "text-white" : "text-slate-600"}`}>{oldPriceText}</div>
      ) : null}
  ...
  const Footer = ({ dark = false }: { dark?: boolean }) => state.showFooter ? (
    <div className={`flex items-center justify-between gap-5 border-t pt-4 ${dark ? "border-white/30 text-white" : "border-slate-300 text-slate-800"}`}>
      ...
      {state.showLogo && brand.logoUrl ? (
        <img src={brand.logoUrl} alt={brand.companyName} className="max-h-14 max-w-[150px] object-contain" />
      ) : null}
    </div>
  ) : null;
  ...
  const commonInfo = (dark = false) => (
    <div className="space-y-4">
      <div className="text-lg font-black uppercase tracking-[0.18em]" style={{ color: state.primaryColor }}>{state.operation}</div>
      <h2 className={`text-5xl font-black uppercase leading-[0.98] ${dark ? "text-white" : "text-slate-950"}`}>{state.title}</h2>
      {state.showLocation && state.location ? <div className={`text-2xl font-semibold ${dark ? "text-white/90" : "text-slate-700"}`}>📍 {state.location}</div> : null}
        ...
        return (
          <div className="relative flex h-full w-full bg-white">
            ...
            <div className="flex w-[34%] flex-col justify-between p-[4%]" style={{ backgroundColor: state.secondaryColor, color: "white" }}>
              ...
              <div className="space-y-6"><PriceBlock dark /><Footer dark /></div>
            </div>
            <StatusStamp />
          </div>
        );
        ...
        return (
          <div className="relative h-full w-full bg-white">
            ...
            <div className="flex h-[30%] items-center justify-between gap-8 px-[5%] py-[3%]">
              <div className="max-w-[65%]">{commonInfo(false)}</div>
              <div className="space-y-5"><PriceBlock /><Footer /></div>
            </div>
            <StatusStamp />
          </div>
        );
        ...
        return (
          <div className="relative h-full w-full bg-white">
            ...
            <div className="flex h-[32%] items-center justify-between gap-8 px-[5%] py-[3%]">
              <div className="max-w-[65%]">{commonInfo(false)}</div>
              <div className="space-y-4"><PriceBlock /><Footer /></div>
            </div>
            <StatusStamp />
          </div>
        );
        ...
        return (
          <div className="relative flex h-full w-full flex-col bg-[#f7f7f4] p-[5%]">
            <div className="mb-[3%] flex items-center justify-between">
              <div className="text-2xl font-black uppercase tracking-[0.25em]" style={{ color: state.primaryColor }}>{state.operation}</div>
              {state.showLogo && brand.logoUrl ? <img src={brand.logoUrl} alt={brand.companyName} className="max-h-16 max-w-[150px] object-contain" /> : null}
            </div>
  ...
  return (
    <div className="space-y-5">
      <header className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-600">La propiedad es la protagonista. Las fotos y placas se procesan localmente y no se guardan.</p>
      </header>
      ...
      <div className="grid gap-5 xl:grid-cols-[390px_minmax(0,1fr)]">
        <aside className="space-y-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <Section title="Formato y plantilla">
            <select className="input" value={state.format} onChange={(e) => update("format", e.target.value as PlateFormat)}>
              {Object.entries(FORMAT_SIZE).map(([key, value]) => <option key={key} value={key}>{value.label}</option>)}
            </select>
            <div className="grid grid-cols-2 gap-2">
              {TEMPLATE_OPTIONS.map((template) => (
                <button key={template.id} type="button" onClick={() => update("templateId", template.id)} className={`rounded-xl border p-3 text-left ${state.templateId === template.id ? "border-slate-950 bg-slate-950 text-white" : "border-slate-200 hover:bg-slate-50"}`}>
          ...
          <Section title="Fotos de la propiedad">
            <input type="file" accept="image/*" multiple onChange={(e) => handleImages(e.target.files)} className="block w-full text-sm" />
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <label className="text-xs text-slate-600">Zoom {state.imageZoom.toFixed(1)}<input type="range" min="1" max="1.8" step="0.1" value={state.imageZoom} onChange={(e) => update("imageZoom", Number(e.target.value))} className="mt-1 w-full" /></label>
            </div>
          </Section>

          <Section title="Aplicar sello a una placa existente">
            <input type="file" accept="image/*" onChange={(e) => handleImages(e.target.files, true)} className="block w-full text-sm" />
            <label className="check"><input type="checkbox" checked={state.useImportedPlateMode} onChange={(e) => update("useImportedPlateMode", e.target.checked)} disabled={!state.importedPlate} /> Usar la placa importada como base</label>
          </Section>
          ...
          <Section title="Datos de la propiedad">
            ...
            <div className="grid grid-cols-[90px_1fr] gap-2"><input className="input" value={state.currency} onChange={(e) => update("currency", e.target.value)} placeholder="USD" /><input className="input" value={state.price} onChange={(e) => update("price", e.target.value)} placeholder="Precio nuevo" /></div>
            ...
            <div className="grid grid-cols-2 gap-2"><input className="input" value={state.coveredArea} onChange={(e) => update("coveredArea", e.target.value)} placeholder="m² cubiertos" /><input className="input" value={state.totalArea} onChange={(e) => update("totalArea", e.target.value)} placeholder="m² totales" /></div>
          ...
          <Section title="Sello y visibilidad">
            <select className="input" value={state.status} onChange={(e) => update("status", e.target.value as PropertyStatus)}>{STATUS_OPTIONS.map((status) => <option key={status || "none"} value={status}>{status || "Sin sello"}</option>)}</select>
          ...
          <Section title="Colores">
            <div className="grid grid-cols-2 gap-3"><ColorInput label="Principal" value={state.primaryColor} onChange={(value) => update("primaryColor", value)} /><ColorInput label="Secundario" value={state.secondaryColor} onChange={(value) => update("secondaryColor", value)} /></div>
        ...
        <main className="min-w-0 rounded-2xl border border-slate-200 bg-slate-100 p-4 shadow-inner">
          <div className="mb-3 flex items-center justify-between text-xs text-slate-500"><span>Vista previa</span><span>{size.width} × {size.height}px</span></div>
          <div className="overflow-auto">
            <div style={{ width: size.width * scale, height: size.height * scale }}>
              <div style={{ transform: `scale(${scale})`, transformOrigin: "top left", width: size.width, height: size.height }}>
                <div ref={canvasRef} style={{ width: size.width, height: size.height }} className="overflow-hidden bg-white font-sans text-slate-950">{renderTemplate()}</div>
              </div>
            </div>
          </div>
        </main>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return <section className="space-y-3 border-b border-slate-100 pb-4 last:border-0"><h2 className="text-sm font-bold text-slate-950">{title}</h2>{children}</section>;
}

function Toggle({ label, value, onChange }: { label: string; value: boolean; onChange: (value: boolean) => void }) {
  return <label className="check rounded-lg border border-slate-200 p-2"><input type="checkbox" checked={value} onChange={(e) => onChange(e.target.checked)} />{label}</label>;
}

function ColorInput({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return <label className="text-xs font-semibold text-slate-600">{label}<div className="mt-1 flex items-center gap-2"><input type="color" value={value} onChange={(e) => onChange(e.target.value)} className="h-10 w-12 rounded border" /><input className="input" value={value} onChange={(e) => onChange(e.target.value)} /></div></label>;
}
