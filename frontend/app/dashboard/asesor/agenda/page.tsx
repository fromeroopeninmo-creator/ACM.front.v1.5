// app/dashboard/asesor/agenda/page.tsx
"use client";
...
export default function AsesorAgendaPage() {
  const { user } = useAuth();
  ...
  const fetchAgendaData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const [actividadesRes, contactosRes] = await Promise.all([
        supabase
  ...
  useEffect(() => {
    const fetchAsesor = async () => {
      ...
      if (user.role !== "asesor") {
        console.error("Usuario sin rol asesor intentando acceder a Agenda");
        setLoading(false);
        return;
      }
      ...
      if (error) {
        console.error("Error buscando asesor para agenda:", error);
        setEmpresaId(null);
        setAsesorId(null);
        setLoading(false);
        return;
      ...
      if (!data) {
        console.error("No se encontró el perfil de asesor para agenda");
        setEmpresaId(null);
        setAsesorId(null);
        setLoading(false);
        return;
  ...
  useEffect(() => {
    if (!empresaId || !asesorId) return;
    fetchAgendaData();
  }, [empresaId, asesorId, fetchAgendaData]);
  ...
  }, [actividades, tipoFiltro, busqueda, contactos]);
  ...
  const guardarActividad = async () => {
    if (!empresaId || !asesorId || saving) return;
  ...
  if (!loading && (!user || user.role !== "asesor" || !empresaId || !asesorId)) {
    return (
      <div className="flex items-center justify-center h-[60vh] text-gray-500">
        No se encontró tu perfil de asesor para usar la agenda.
      </div>
    );
  }
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-[11px] text-slate-500">
              {formatDateShort(a.fecha_programada)} · {formatTime(a.hora)} · {a.duracion_minutos ?? 30} min
            </p>
      ...
      <div className="max-w-6xl xl:max-w-7xl mx-auto px-4 py-6 space-y-6">
        <header className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm md:text-base text-slate-600 max-w-2xl">
              Organizá tus tareas, reuniones, prelistings y seguimientos. Abrí WhatsApp del cliente y sincronizá tus eventos con Google Calendar.
            </p>
          </div>
        ...
        <section className="grid gap-4 md:grid-cols-4">
          ...
          <div className="rounded-xl border border-gray-200 bg-white px-4 py-3">
            ...
            <p className="mt-1 text-[11px] text-slate-500">Según tipo y búsqueda.</p>
          </div>
          <div className="rounded-xl border border-gray-200 bg-white px-4 py-3">
