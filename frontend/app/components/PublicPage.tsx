...
export type ToolBlock = { eyebrow: string; title: string; description: string; benefits: string[]; image: string; imageAlt: string; accelerator?: boolean; note?: string; reportHref?: string; reportLabel?: string };
  ...
  return <PublicShell>
    ...
    <div className="mx-auto max-w-6xl px-5 py-16 sm:px-8 lg:px-14">
      <div className="space-y-20">{tools.map((tool, index) => <article key={tool.title} className={`grid items-center gap-10 lg:grid-cols-2 ${index % 2 ? "lg:[&>*:first-child]:order-2" : ""}`}>
        <div><div className="flex flex-wrap items-center gap-3"><span className="text-xs font-semibold uppercase tracking-[.2em] text-[#E6A930]">{tool.eyebrow}</span>{tool.accelerator && <span className="rounded-full border border-[#E6A930]/50 bg-[#E6A930]/10 px-3 py-1 text-xs font-semibold text-[#f2c76d]">Acelerador de venta</span>}</div><h2 className="mt-4 text-3xl font-semibold sm:text-4xl">{tool.title}</h2><p className="mt-5 text-base leading-7 text-neutral-300">{tool.description}</p><ul className="mt-6 grid gap-3">{tool.benefits.map((benefit) => <li key={benefit} className="flex gap-3 text-sm leading-6 text-neutral-200"><span className="mt-1 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#E6A930] text-black"><PublicIcon name="check" className="h-3.5 w-3.5"/></span>{benefit}</li>)}</ul>{tool.note && <p className="mt-5 rounded-xl border border-white/10 bg-white/[.04] p-4 text-xs leading-5 text-neutral-400">{tool.note}</p>}{tool.reportHref && <a href={tool.reportHref} target="_blank" rel="noreferrer" className="mt-6 inline-flex items-center gap-2 rounded-full border border-[#E6A930]/45 bg-[#E6A930]/10 px-5 py-2.5 text-sm font-semibold text-[#f2c76d] transition hover:border-[#E6A930] hover:bg-[#E6A930]/15">{tool.reportLabel ?? "Ver informe de ejemplo"} <PublicIcon name="arrow" className="h-4 w-4"/></a>}</div>
