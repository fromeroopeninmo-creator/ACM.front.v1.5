...
export async function GET(req: NextRequest) {
  try {
    if (!GOOGLE_CLIENT_ID || !GOOGLE_REDIRECT_URI) {
      return jsonError("Faltan GOOGLE_CLIENT_ID o GOOGLE_CALENDAR_REDIRECT_URI.", 500);
    }

    const user = await getUserFromRequest(req);
    if (!user) {
      return jsonError("No autenticado. Abrí la conexión desde VAI Prop con tu sesión activa.", 401);
    }

    const returnTo = req.nextUrl.searchParams.get("returnTo") || "/dashboard/empresa/agenda";
    const state = signState({ user_id: user.id, returnTo, ts: Date.now() });
    ...
    return jsonError(err?.message || "No se pudo iniciar la conexión con Google.", 500);
  }
}
