...
export default function EmpresaCuentaPage() {
  ...
  const getAccessTokenOrThrow = async () => {
    ...
    if (!session?.access_token) {
      throw new Error("No se encontró una sesión activa. Volvé a iniciar sesión.");
    }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("🔒 usando API segura /api/empresa/update");
      console.log("🧪 Payload hacia API:", { update });
      ...
      if (!res.ok) {
        ...
        throw new Error(
          typeof j?.error === "string"
            ? j.error
            : j?.error?.message || "Error en actualización"
        );
      }
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      // 2) Obtener URL pública base, que es la que guardamos en BD
      const {
        data: { publicUrl },
      } = supabase.storage.from("logos_empresas").getPublicUrl(filePath);

      // 3) Actualizar en DB vía API server-side segura.
      // Ya no mandamos user_id: el backend resuelve la empresa por el token.
      const res = await fetch("/api/empresa/update", {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
      ...
      if (!res.ok) {
        ...
        throw new Error(
          typeof j?.error === "string"
            ? j.error
            : j?.error?.message || "Error en actualización de logo"
        );
      }

      // 4) Cache-busting para el preview inmediato.
      // No cambiamos lo guardado en BD.
      const bustedUrl = `${publicUrl}?v=${Date.now()}`;

      // 5) Actualizar UI inmediata
      mutate(
        {
          ...(formData as Record<string, any>),
          logo_url: bustedUrl,
      ...
      // 6) Revalidar datos desde BD
      await mutate();

      setMessage("✅ Logo actualizado correctamente.");
    } catch (err: any) {
      console.error("Error subiendo logo:", err);
      setMessage(`❌ ${err?.message || "Error al subir el logo."}`);
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleAccountUpdate = async (e: React.FormEvent) => {
    ...
    if (!user) {
      setAccountMessage("❌ No se encontró usuario autenticado.");
      return;
    }
