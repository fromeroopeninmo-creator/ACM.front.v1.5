...
/* ===================== Componente ===================== */
export default function CashflowEmpresaClient({
  empresaId,
  initialDesde,
  initialHasta,
}: {
  ...
  const canQuery = useMemo(() => !!desde && !!hasta && !!empresaId, [desde, hasta, empresaId]);
  ...
  async function fetchAll() {
    ...
    try {
      const [k, m, s] = await Promise.all([
        getCashflowKpis({ desde, hasta, empresaId }),
        getCashflowMovimientos({
          desde,
          hasta,
          empresaId,
  ...
  return (
    <section className="space-y-6">
      ...
      {/* KPIs */}
      <section className="grid grid-cols-1 sm:grid-cols-2 xl-grid-cols-4 xl:grid-cols-4 gap-4">
